"""
GPU Benchmark Suite for RAG Performance Testing
================================================
Standardized benchmark for comparing different GPUs.

Usage:
    python benchmark.py
    python benchmark.py --gpu-name "RTX 4090"
...
# Standard test scenarios (industry standard load levels)
LOAD_LEVELS = {
    "smoke": [1, 2],                          # Basic functionality
    "load": [5, 10, 20, 30],                  # Normal expected load
    "stress": [50, 75, 100],                  # Peak load
    "spike": [150, 200],                      # Sudden traffic spike
}
...
def monitor_gpu_during_test(duration: float, interval: float = 0.5) -> dict:
    """Monitor GPU metrics during test execution"""
    samples = []
    start = time.time()
    ...
    return {
        "avg_utilization": round(statistics.mean(s["utilization"] for s in samples), 1),
        "max_utilization": max(s["utilization"] for s in samples),
        "avg_memory_mb": round(statistics.mean(s["memory_used"] for s in samples), 0),
        "peak_memory_mb": max(s["memory_used"] for s in samples),
        "avg_power_watts": round(statistics.mean(s["power"] for s in samples), 1),
        "max_power_watts": round(max(s["power"] for s in samples), 1),
        "max_temperature_c": max(s["temperature"] for s in samples)
    }
...
def calculate_metrics(times: List[float], total_time: float, num_users: int, successful: int) -> dict:
    ...
    return {
        ...
        "apdex_score": calculate_apdex(times, threshold=5.0)
    }

def calculate_apdex(times: List[float], threshold: float) -> float:
    """
    Calculate Apdex (Application Performance Index) score.
    Score: 0.0 (unacceptable) to 1.0 (excellent)
    """
    if not times:
        return 0
    ...
    apdex = (satisfied + tolerating * 0.5) / len(times)
...
def warmup():
    ...
    for i in range(WARMUP_REQUESTS):
        try:
            requests.post(API_URL, json={"question": BENCHMARK_QUESTIONS[0]}, timeout=60)
...
def run_benchmark(gpu_name: str = None, test_type: str = "load"):
    ...
    if gpu_info:
        ...
        print(f"Current Usage: {gpu_info.memory_used_mb} MB ({gpu_info.utilization_percent}%)")
    else:
    ...
    try:
        ...
        print("WARNING: vLLM health check failed (may still work)")
    ...
    for num_users in load_levels:
        ...
        throughputs = [r["metrics"]["throughput_rps"] for r in run_results]
        aggregated_metrics["throughput_variance"] = round(statistics.stdev(throughputs), 3) if len(throughputs) > 1 else 0
        ...
        all_results.append({
            ...
            "individual_runs": [{"metrics": r["metrics"]} for r in run_results]
        })
        
        print(f"\n  Aggregated Results ({REPEAT_COUNT} runs, {total_requests} total requests):")
        print(f"    Throughput: {aggregated_metrics['throughput_rps']:.2f} req/s")
        print(f"    Avg Latency: {aggregated_metrics['avg_latency_s']:.2f}s")
        print(f"    P50/P95/P99: {aggregated_metrics['p50_s']:.2f}s / {aggregated_metrics['p95_s']:.2f}s / {aggregated_metrics['p99_s']:.2f}s")
        print(f"    Apdex Score: {aggregated_metrics['apdex_score']:.3f}")
        print(f"    Success Rate: {aggregated_metrics['success_rate_pct']:.1f}%")
...
def generate_summary(results: List[dict]) -> dict:
    ...
    if len(results) >= 2:
        ...
        throughput_change = (last["metrics"]["throughput_rps"] - first["metrics"]["throughput_rps"]) / first["metrics"]["throughput_rps"] * 100
    else:
        latency_increase = 0
        throughput_change = 0
    ...
    return {
        ...
        "throughput_scaling_pct": round(throughput_change, 1),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpu-name", type=str, help="Override GPU name for report")
    parser.add_argument("--test-type", type=str, default="load",
                        choices=["smoke", "load", "stress", "spike"],
                        help="Test type: smoke, load, stress, or spike")
